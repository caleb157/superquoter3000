...
export const ProjectRfqTab = ({ projectId }: Props) => {
  ...
  const fetchRfqs = async () => {
    const { data } = await (supabase as any).from('vendor_rfqs')
      .select('*')
  ...
  const handleGenerate = async (type: string) => {
    ...
    try {
      ...
      toast.success(`${TYPE_LABELS[type]} Vendor RFQ created with ${result.items.length} items`);
      if (rfqId) navigate(`/vendor-rfq/${rfqId}`);
    } catch (err: any) {
      toast.error(err.message || 'Failed to generate Vendor RFQ');
    } finally {
      setGenerating(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <Card><CardContent className="py-8 text-center text-sm text-muted-foreground">
          ...
          <p>No Vendor RFQs for this project yet.</p>
        </CardContent></Card>
      ) : (
        <Card>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="text-xs">Vendor RFQ #</TableHead>
