...
export function BulkSetProductTypeDialog({ open, onOpenChange, selectedProductIds, onApplied }: Props) {
  ...
  const apply = async () => {
    ...
    setApplying(false);
    const label = types.find(t => t.id === value)?.name || 'product type';
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!applying) onOpenChange(o); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Updates the product type on each product and recomputes costing (finishing chemicals defaults depend on type).
          </DialogDescription>
        </DialogHeader>
