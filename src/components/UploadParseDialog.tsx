...
export function UploadParseDialog({ open, onOpenChange, inquiryId, productTypes, onProductsCreated }: UploadParseDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Use shared, cached global default so repeated imports don't refetch.
      const { getCachedMcHeightBuffer } = await import('@/lib/product-defaults');
      const gsMcHBuffer = await getCachedMcHeightBuffer();
      ...
      for (const p of toImport) {
        ...
        if (p.cogs_rows && p.cogs_rows.length > 0) {
          ...
          const defaultCogs = [
            { product_id: prod.id, cogs_type: 'Raw Piece', component_name: 'Raw Piece', sort_order: 0 },
            { product_id: prod.id, cogs_type: 'Subcontracting', component_name: 'Subcontracting', sort_order: 1 },
            ...
            ...hwRows,
            { product_id: prod.id, cogs_type: 'Accessories', component_name: 'Accessory', waste_factor: 0.05, sort_order: 20 },
          ];
