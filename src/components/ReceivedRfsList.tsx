...
export function ReceivedRfsList({ inquiryId }: { inquiryId: string }) {
  ...
  const save = async () => {
    ...
    if (editing) {
      const { error } = await (supabase as any).from('inquiry_received_rfs')
  ...
  const remove = async (id: string) => {
    ...
    await (supabase as any).from('inquiry_received_rfs').delete().eq('id', id);
    load();
  };
  ...
  return (
    <Card>
      <CardHeader className="pb-2 flex flex-row items-center justify-between">
        <CardTitle className="text-sm">Customer sample requests received</CardTitle>
        <Button size="sm" variant="outline" className="h-8 gap-1.5 text-xs" onClick={openNew}>
          <Plus className="h-3.5 w-3.5" /> Log received RFS
        </Button>
      </CardHeader>
      <CardContent>
        {rows.length === 0 ? (
          <div className="text-sm text-muted-foreground text-center py-4">No sample requests logged yet.</div>
        ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>{editing ? 'Edit received RFS' : 'Log received RFS'}</DialogTitle></DialogHeader>
