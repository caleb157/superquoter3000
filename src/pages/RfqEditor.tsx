...
import { ArrowLeft, Save, Download, Link2, Send, CheckCircle, Plus, Trash2, CalendarIcon } from 'lucide-react';
...
const RfqEditor = () => {
  ...
  const updateItem = (itemId: string, field: string, value: any) => {
    setItems(prev => prev.map(item => {
      ...
      // Auto-recalculate target_price when discount changes and value isn't manually overridden
  ...
  const save = async () => {
    ...
    try {
      ...
      const { error: rfqErr } = await (supabase as any).from('rfqs').update({
        ...
        vendor_address: rfq.vendor_address,
      ...
      // Delete removed items, upsert existing + new
      const existingIds = items.filter(i => !i._isNew).map(i => i.id);
      // Delete items not in our list
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const row = {
          ...
          vendor_price: item.vendor_price || null,
          notes: item.notes || null,
          sort_order: i,
        };

        if (item._isNew) {
          await (supabase as any).from('rfq_line_items').insert(row);
        } else {
          await (supabase as any).from('rfq_line_items').update(row).eq('id', item.id);
        }
      }
  ...
  const copyShareLink = () => {
    ...
    const url = `${window.location.origin}/rfq/view/${rfq.share_token}`;
  ...
  const downloadPdf = () => {
    if (!rfq?.share_token) return;
    const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-rfq-pdf?token=${rfq.share_token}`;
          ...
          <CardContent className="pt-4 space-y-3">
            ...
            <div className="grid grid-cols-4 gap-3">
              <div>
                ...
                <Input className="h-8 text-sm" value={rfq.vendor_name || ''} onChange={e => updateRfqField('vendor_name', e.target.value)} placeholder="Vendor name" />
              </div>
              <div>
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <Input
            ...
            onChange={e => {
              ...
              updateRfqField('discount_percent', isNaN(val) ? null : val);
            }}
            onBlur={() => {
              if (rfq.discount_percent != null) recalculateTargetPrices(rfq.discount_percent);
            }}
          />
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-xs w-16">Units</TableHead>
                  <TableHead className="text-xs text-right w-24">Est. Cost</TableHead>
                  <TableHead className="text-xs text-right w-24">Target Price</TableHead>
                  <TableHead className="text-xs text-right w-24">Vendor Price</TableHead>
              ...
              <TableBody>
                {items.map((item, idx) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <Input className="h-7 text-xs border-transparent hover:border-input" value={item.description || ''} onChange={e => updateItem(item.id, 'description', e.target.value)} />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Input className="h-7 text-xs border-transparent hover:border-input" value={item.notes || ''} onChange={e => updateItem(item.id, 'notes', e.target.value)} />
                    </TableCell>
                    <TableCell>
